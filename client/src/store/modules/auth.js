...
const actions = {
  AUTH_TRANSITION: (context, action) => {
    transition(authMachine, context, action);
  },
  CHECK_FOR_AUTH: async ({ dispatch }) => {
    console.log("CALLED");
    const token = localStorage.getItem("token");
    if (!token) {
      dispatch("AUTH_TRANSITION", "ERROR");
      return;
    }
    const isAuthQuery = {
      query: `
    ...
    try {
      ...
      dispatch("AUTH_TRANSITION", data.data.isAuth ? "SUCCESS" : "ERROR");
      return;
    } catch (err) {
      dispatch("AUTH_TRANSITION", "ERROR");
    }
  ...
  POST_USER: async ({ state, dispatch, commit }) => {
    fetch(process.env.VUE_APP_API, {
      ...
      .then(data => {
        ...
        // Transition state
        dispatch("AUTH_TRANSITION", "SUCCESS");
      })
      .catch(err => {
        // Update error

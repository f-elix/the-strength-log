		...
		states: {
			...
			loading: {
				...
				states: {
					fetchingSession: {
						invoke: {
							src: (_, event) => getSession(event.params),
							onDone: {
								target: "#displaying",
					...
					savingSession: {
						invoke: {
							src: (_, event) => saveSession(event.params),
							onDone: {
								target: "#displaying",
								actions: ["routeSession", "updateSessionData", "updateCurrentSession"]
							},
							onError: "#editing"
						}
					},
					deletingSession: {
						invoke: {
							src: (_, event) => deleteSession(event.params),
							onDone: {
								target: "#init",
		...
		actions: {
			...
			deleteSet: assign({
				sessionData: (context, event) => {
					...
					const movementIndex = exercise.movements.indexOf(movement);
					const exerciseIndex = sessionData.exercises.indexOf(exercise);
					const filteredSets = sessionData.exercises[exerciseIndex].movements[movementIndex].sets.filter(
						set => {
							return set.id !== setId;
						}
					);
